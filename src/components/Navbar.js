...
import { HomeIcon, MoonIcon, ProjectsIcon, SunIcon, SupportIcon, UpdateIcon, UserIcon } from './Icons'
...
const CustomLink = ({ href, title, className=""}) => {
  ...
  return (
    <Link href={href} className={`${className} relative group`}>
      ...
      <span className={`h-[1.5px] inline-block  bg-dark absolute w-0 left-0 -bottom-0.5
...
>
      ...
      <div className='flex justify-center items-center '>
        <div>
          <button
            ...
            className='mr-4  flex items-center justify-center p-1 border-none' aria-label="Dark Mode"
          >
        ...
        <div>
        <button className='flex-col justify-center items-center ' aria-label="Menu Bar" onClick={handleClick}>
          <span className={`bg-dark dark:bg-light block transition-all duration-300 ease-out h-1.5 w-7 rounded-sm  ${isOpen ? 'rotate-45 translate-y-1' : '-translate-y-0.5'}`}></span>
          <span className={` dark:bg-light block transition-all duration-300 ease-out rounded-sm my-0.5 ${isOpen ? 'opacity-0' : 'opacity-100'}`}></span>
          <span className={`bg-dark dark:bg-light block transition-all duration-300 ease-out  h-0.5 w-7 rounded-sm  ${isOpen ? '-rotate-45 -translate-y-1' : 'translate-y-0.5'}`}></span>
        </button>
        </div>
      </div>
    </div>
    ...
    <Logo />
      ...
      <nav className='flex'>
        ...
        <button
            onClick={() => setMode(mode === "light" ? "dark" : "light")}
            className='mr-4 flex items-center justify-center rounded-full p-1' aria-label="Dark Mode"
          >
            {
              mode === "dark" ?
              <SunIcon className={"fill-dark"} />
          ...
          <button className='bg-black hover:bg-black/80 text-white text-md font-bold px-4 rounded dark:bg-light dark:text-dark'>
            <a target="_blank" href="mailto:nextdev0.002@gmail.com"> Say Hi, </a>
          </button>
